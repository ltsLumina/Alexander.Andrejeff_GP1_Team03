...
public static class VHierarchy
{
    static void RowGUI(int instanceId, Rect rowRect)
    {
        ...
        else
        {
            ...
            if (iScene != -1) SceneRowGUI(EditorSceneManager.GetSceneAt(iScene), rowRect);
        }
    }
    ...
    static void GameObjectRowGUI(GameObject go, Rect rowRect)
    {
        ...
        void hydrateData()
        {
            void scene()
            {
                ...
                if (!data.sceneDatasByGuid.ContainsKey(guid)) data.sceneDatasByGuid[guid] = new SceneData();
                ...
                foreach (var r in deletedGids) sceneData.goDatasByGlobalId.Remove(r);
            }
    ...
    static void CheckShortcuts()
    {
        ...
        void toggleActive()
        {
            ...
            var gos = Selection.gameObjects.Contains(hoveredGo)
        ...
        void delete()
        {
            ...
            foreach (var r in gos) Undo.DestroyObjectImmediate(r);
        ...
        void collapseEverythingElse()
        {
            ...
            expandQueue_toAnimate = toCollapse.Select(r => new ExpandQueueEntry(r.GetInstanceID(), false)).Append(new ExpandQueueEntry(hoveredGo.GetInstanceID(), true)).OrderBy
                (r => VisibleRowIndex(r.instanceId)).ToList();
    ...
    static void UpdateExpandQueue()
    {
        ...
        if (!expandQueue_toAnimate.Any())
        {
            ...
            foreach (var r in expandQueue_toCollapseAfterAnimation) SetExpanded(r.GetInstanceID(), false);
    ...
    {
        if (!Application.isPlaying) expandedIds = hierarchyWindow?.GetFieldValue("m_SceneHierarchy").GetFieldValue("m_TreeViewState").GetPropertyValue<List<int>>("expandedIDs");
    ...
    public static GUIContent GetComponentIcon(Component component)
    {
        ...
        if (!componentIconsByType.ContainsKey(component.GetType())) componentIconsByType[component.GetType()] = new GUIContent(EditorGUIUtility.ObjectContent(component, component.GetType()).image);
    ...
    static void OnProjectWasLoaded()
    {
        ...
        foreach (var sceneData in data.sceneDatasByGuid.values) sceneData.goDatasByInstanceId.Clear();
    }
